// /blog — the blog index page. Same JSON pipeline as the locations
// and services hubs. The 20 individual posts live at /blog/[slug].
...
async function load(): Promise<Data | null> {
  try {
    const file = path.join(process.cwd(), "data", "blog", "index.json")
...
export async function generateMetadata(): Promise<Metadata> {
  const d = await load()
  if (!d) return { title: "Blog" }
  return {
    title: d.title,
    description: d.description,
    keywords: d.keywords,
    alternates: { canonical: d.canonical || "/blog" },
    openGraph: {
      type: "website",

// /services — the overall services hub. Same JSON pipeline as locations:
...
async function load(): Promise<Data | null> {
  try {
    const file = path.join(process.cwd(), "data", "services", "index.json")
...
export async function generateMetadata(): Promise<Metadata> {
  const d = await load()
  if (!d) return { title: "Services" }
  return {
    title: d.title,
    description: d.description,
    keywords: d.keywords,
    alternates: { canonical: d.canonical || "/services" },
    openGraph: {
      type: "website",

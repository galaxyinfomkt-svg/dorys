...
function updateFooterInFile(filePath) {
  ...
  // Check if already has complete footer
  if (content.includes('footer__cta-title')) {
  ...
  if (footerRegex.test(content)) {
    content = content.replace(footerRegex, COMPLETE_FOOTER);
...
function updateAllFooters() {
  ...
  // Update main pages
  const mainPages = ['about.html', 'contact.html', 'reviews.html', 'privacy.html', 'terms.html'];
  mainPages.forEach(page => {
    ...
    if (fs.existsSync(filePath)) {
      if (updateFooterInFile(filePath)) {
        ...
        skipped++;
        console.log('Skipped (already updated): ' + page);
      }
    }
  });

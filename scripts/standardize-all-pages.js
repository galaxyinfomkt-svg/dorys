/**
 * COMPREHENSIVE STANDARDIZATION SCRIPT
 * 1. Makes menu (top-bar + header) identical to homepage across ALL pages
 * 2. Makes footer identical to homepage across ALL pages
 * 3. Fixes ALL remaining old service references
 */
...
// ============================================================
// STANDARD HEADER (from homepage, with absolute paths)
// ============================================================
const STANDARD_TOPBAR_HEADER = `  <div class="top-bar">
    <div class="container">
      <div class="top-bar__contact">
        <a href="tel:+19783078107" class="top-bar__item" data-track="phone-topbar">
        ...
        <a href="mailto:contact@doryscleaningservices.com" class="top-bar__item" data-track="email-topbar">
          <svg class="top-bar__icon" viewBox="0 0 24 24" fill="currentColor">
  ...
  <header class="header">
    <div class="container header__wrapper">
      <a href="/" class="header__logo">
        <img src="/assets/images/logo/logo-original.jpg" alt="Dorys Healthcare Environmental Services Logo" width="180" height="60">
      </a>
        ...
        <ul class="nav-list">
          ...
          <li class="nav-item has-dropdown">
            ...
            <div class="nav-dropdown">
              <a href="/services/medical-office-cleaning" class="nav-dropdown__link">
              ...
              <a href="/services/clinic-outpatient-sanitation" class="nav-dropdown__link">
              ...
              <a href="/services/assisted-living-senior-care" class="nav-dropdown__link">
              ...
              <a href="/services/infection-control-disinfection" class="nav-dropdown__link">
              ...
              <a href="/services/compliance-scheduled-sanitation" class="nav-dropdown__link">
          ...
          <li class="nav-item has-dropdown">
            ...
            <div class="nav-dropdown nav-dropdown--mega">
              ...
              <div class="nav-dropdown__grid">
                ...
                <a href="/locations/hudson-ma" class="nav-dropdown__link">Hudson</a>
                <a href="/locations/westborough-ma" class="nav-dropdown__link">Westborough</a>
                <a href="/locations/natick-ma" class="nav-dropdown__link">Natick</a>
                <a href="/locations/newton-ma" class="nav-dropdown__link">Newton</a>
                <a href="/locations/wellesley-ma" class="nav-dropdown__link">Wellesley</a>
                <a href="/locations" class="nav-dropdown__link text-primary font-semibold">View All Cities &rarr;</a>
              </div>
            </div>
          </li>
          <li class="nav-item">
...
// ============================================================
// STANDARD FOOTER (from homepage, with absolute paths)
// ============================================================
      ...
      <div class="footer__grid">
        ...
        <div class="footer__col">
          ...
          <div class="footer__cities-more">
            <a href="/locations">View All 100+ Cities &rarr;</a>
          </div>
        </div>
...
function findAllHtmlFiles(dir, fileList = []) {
  ...
  for (const entry of entries) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      if (['.git', 'node_modules', '.vercel', '.claude', 'scripts'].includes(entry)) continue;
      findAllHtmlFiles(fullPath, fileList);
...
function replaceHeaderFooter(content, filePath) {
  let modified = content;

  // Replace top-bar through end of </header>
  // Match from <div class="top-bar"> to </header>
  const headerRegex = /[ \t]*<div class="top-bar">[\s\S]*?<\/header>/;
  if (headerRegex.test(modified)) {
    modified = modified.replace(headerRegex, STANDARD_TOPBAR_HEADER);
  }

  // Replace footer
  const footerRegex = /[ \t]*<footer class="footer"[\s\S]*?<\/footer>/;
  if (footerRegex.test(modified)) {
    modified = modified.replace(footerRegex, STANDARD_FOOTER);
  }

  return modified;
}

function fixOldServiceText(content, filePath) {
  let modified = content;
...
const allFiles = findAllHtmlFiles(ROOT);
const SKIP_FILES = ['index.html']; // Homepage already correct
...
for (const filePath of allFiles) {
  try {
    ...
    // Skip homepage for header/footer (already correct)
    const isHomepage = relPath === 'index.html';

    if (!isHomepage) {
      const afterHeader = replaceHeaderFooter(modified, filePath);
      if (afterHeader !== modified) {
        if (afterHeader.includes(STANDARD_TOPBAR_HEADER.substring(0, 50))) headerUpdated++;
        if (afterHeader.includes('footer__credentials')) footerUpdated++;
        modified = afterHeader;
      }
    }

    // Fix old service text in ALL files (including homepage)
    const afterText = fixOldServiceText(modified, filePath);
    if (afterText !== modified) {
      textFixed++;
      modified = afterText;
    }
...
console.log(`  Headers updated: ${headerUpdated}`);
console.log(`  Footers updated: ${footerUpdated}`);
console.log(`  Text references fixed: ${textFixed}`);
